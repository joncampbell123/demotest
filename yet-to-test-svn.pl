...
while (my $path = <X>) {
    ...
    # TEMP: we only want 1993 and earlier demos
    next unless ($path =~ m/\/199[0-3]\// || $path =~ m/\/198[0-9]\//);
    ...
    next if (
        ( -f "$path/__PASS_SVN__" && ( -s "$path/__PASS_SVN__") > 4) ||
        ( -f "$path/__FAIL_SVN__" && ( -s "$path/__FAIL_SVN__") > 4)
    );
